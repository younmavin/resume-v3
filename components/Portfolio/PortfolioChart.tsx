    ...
    <div className="portfolio-chart cont">
      ...
      <Link className="chart-bx" href="/portfolio">
        ...
        <div className="chart-inner">
          <div className="chart">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" labelLine={false} label={renderCustomLabel} stroke="var(--bg-card)">
                  {data.map((entry, i) => (
            ...
            {data.map((entry, i) => (
              <li key={i}>
                <span
                  className="dot"
                  style={{
                    background: COLORS[entry.name] ?? 'var(--chart-etc)',
                  }}
                />
                ...
                <span
                  className="count"
                  style={{
                    color: COLORS[entry.name] ?? 'var(--chart-etc)',
                  }}
                >
                  {entry.value}개
                </span>
        ...
        <p className="deps">
          ...
          <br />
          성격이 중복되는 프로젝트와 URL이 확인되지 않는 사이트는 제외했습니다.
        </p>
      </Link>
    </div>
  )
